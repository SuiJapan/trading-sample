...
import { ACTIVE_NETWORK, getActiveAddress, signAndExecute } from "../utils/sui-utils";

// a simple example of objects by creating N amount of bears.
const createDemoLockedObjects = async (totalBears: number) => {
	if (totalBears < 5)
		throw new Error("Please create at least 5 bears to run this script.");
	...
	if (!res.objectChanges || res.objectChanges.length === 0)
		throw new Error(
			"Something went wrong while creating demo bears & locked objects.",
		);
