...
import { getActiveAddress, getClient, signAndExecute } from "../utils/sui-utils";
...
const createEscrows = async (total: number) => {
	...
	if (
		!lockedObjects ||
		!ownedBears ||
		lockedObjects.length < total ||
		ownedBears.length < total
	)
		throw new Error(
	...
	for (const tuple of tuples) {
		if (!tuple.bear) break;
		if (!tuple.bear.objectId)
			throw new Error("bear.objectId is not defined. Does not make sense!");
